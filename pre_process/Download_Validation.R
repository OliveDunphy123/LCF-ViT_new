...
collect_data <- function(roi_sf, multicores=1) {
  ...
  s2_validation_mpc <- sits::sits_cube(
    source     = "MPC",
...
download_data <- function(s2_validation_mpc, roi_sf, output_dir, location_id, multicores=1) {
  ...
  results <- pblapply(
    ...
    FUN = function(i) {
      ...
      tryCatch({
        ...
        s2_validation_mpc <- collect_data(roi_sf, multicores = 1)  # Use 1 core per worker
        if (is.null(s2_validation_mpc)) {
          return(list(location_id = location_id, status = "no_data", message = "No data available for this location"))
        }
        location_output_dir <- download_data(s2_validation_mpc, roi_sf, output_dir, location_id, multicores = 1)  # Use 1 core per worker
        
        rm(roi_sf, s2_validation_mpc)
        gc()
...
# Main execution
validation_africa <- read.csv("~/R/Data/Raw/validation_africa.csv")
sample_sites <- validation_africa %>% 
...
rm(ChangeSamples, NoChangeSamples, validation_africa, ChangeIDs)
gc()
#sample_sites <- sample_sites[1:1000, ]
  
output_dir <- "/mnt/guanabana/raid/shared/dropbox/QinLennart/Validation_africa_new"
