...
class BaseDataset(Dataset):
    ...
    def _validate_ground_truth(self):
        ...
        for loc_id, files in gt_by_location.items():
            ...
            for i in range(1, 8):  # 7 fraction files per location
                fraction_file = f"stacked_{loc_id}_fraction_{i}.tif"
                if fraction_file not in files:
                    missing_fractions.append(fraction_file)
                else:
                    fraction_files.append(fraction_file)
            if missing_fractions:
                self.skipped_locations[loc_id] = f"Missing GT files: {missing_fractions}"
            ...
            # After we know there are 7 fraction files for this location, check first file for years and shape
            first_path = os.path.join(self.gt_path, fraction_files[0])
            
            try:
                with rasterio.open(first_path) as src:
                    data = src.read()
                    # print(f"Inital data shape: {data.shape}")
                    if self.split in ["Val_set", "Test_set"] and data.shape[0] == 5:
                        data = data[:4]
                        # print(f"After slicing: {data.shape}")
                    if data.shape[0] !=4:  # Should have 4 years, but val_set and test_set have 5 years
        ...
        print(f"\nValidation Results:")
        print(f"Valid locations: {len(valid_gt)}")
        print(f"Skipped locations: {len(self.skipped_locations)}")
    ...
    def _validate_sentinel_data(self, valid_locations):
        ...
        print(f"Validating Sentinel data for {self.temporal_mode} mode...")
        valid_pairs = []
        locations_with_missing_bands = set()
            ...
            for period in time_periods:
                for band in self.bands:
                    ...
                    if filename not in files:
                        ...
                        missing_bands.add(band)
                        
            if not location_complete:
                locations_with_missing_bands.add(loc_id)
                self.skipped_locations[loc_id] = f"Missing bands: {', '.join(sorted(missing_bands))}"
                self.skip_statistics['incomplete_sentinel'] += len(time_periods)  # Count all months as incomplete
                continue  # Skip to next location

            # Only process complete locations
            for period in time_periods:
                sample_file = f"{period}_{self.bands[0]}.tif"
                try:
                    with rasterio.open(os.path.join(sentinel_loc_path, sample_file)) as src:
        ...
        print(f"\nValidation Summary:")
        print(f"Total locations checked: {len(valid_locations)}")
        print(f"Locations with missing bands: {len(locations_with_missing_bands)}")
        print(f"Total incomplete months: {self.skip_statistics['incomplete_sentinel']}")
        print(f"Valid location-time pairs: {len(valid_pairs)}")
            ...
            for path in gt_paths:
                with rasterio.open(path) as src:
                    ...
                    if self.temporal_mode == "monthly":
                        ...
                        data = torch.cat([torch.from_numpy(d) for d in year_data], dim=0)  # [42, 5, 5]   
...
def create_yearly_15_dataloader(base_path, split="Training", batch_size=32, num_workers=4, debug=False):
    dataset = BaseDataset(base_path, split=split, temporal_mode="yearly", debug=debug)
    wrapped_dataset = DatasetWrapper(dataset, 'full')
    return DataLoader(wrapped_dataset, batch_size=batch_size, 
                     shuffle=(split=="Training"), num_workers=num_workers, pin_memory=True)

def create_yearly_5_dataloader(base_path, split="Training", batch_size=32, num_workers=4, debug=False):
    dataset = BaseDataset(base_path, split=split, temporal_mode="yearly", debug=debug)
    wrapped_dataset = DatasetWrapper(dataset, 'crop')
    return DataLoader(wrapped_dataset, batch_size=batch_size, 
                     shuffle=(split=="Training"), num_workers=num_workers, pin_memory=True)

def create_monthly_15_dataloader(base_path, split="Training", batch_size=32, num_workers=4, debug=False):
    dataset = BaseDataset(base_path, split=split, temporal_mode="monthly", debug=debug)
    wrapped_dataset = DatasetWrapper(dataset, 'full')
    # Calculate optimal number of workers based on CPU cores
    num_workers = min(16, os.cpu_count() or 1)  # Use up to 12 workers
    ...
    return DataLoader(wrapped_dataset, batch_size=batch_size, 
                     shuffle=(split=="Training"), num_workers=num_workers, pin_memory=True, persistent_workers=True, prefetch_factor=4, drop_last=True)

def create_monthly_5_dataloader(base_path, split="Training", batch_size=32, num_workers=4, debug=False):
    dataset = BaseDataset(base_path, split=split, temporal_mode="monthly", debug=debug)
    wrapped_dataset = DatasetWrapper(dataset, 'crop')
    # Calculate optimal number of workers based on CPU cores
    num_workers = min(16, os.cpu_count() or 1)  # Use up to 12 workers
    ...
    try:
        ...
        for name, loader_fn in loaders.items():
            ...
            for split in ["Training", "Val_set", "Test_set"]:
                ...
                print(f"{split} {name}: sentinel shape {batch['sentinel'].shape}")
