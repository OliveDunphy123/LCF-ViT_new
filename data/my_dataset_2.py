...
class TrainingDataset(Dataset):
    def __init__(self, base_path, temporal_mode="monthly", debug=False):
        ...
        # Set paths
        self.sentinel_path = os.path.join(base_path, "training_subset_stacked", temporal_mode)
    ...
    def _get_unique_ids(self):
        ...
        for loc_id in gt_locations:
            ...
            if os.path.exists(sentinel_loc_path):
                unique_ids.append(loc_id)
    ...
    def _load_sentinel(self, loc_id):
        """
        Load stacked Sentinel data for all bands
        ...
        for band_idx, band in enumerate(self.bands):
            file_path = os.path.join(self.sentinel_path, loc_id, f"{band}_stacked.tif")
            
            try:
                with rasterio.open(file_path) as src:
                    # Read all temporal data at once since it's stacked
                    band_data = src.read()  # Should be [T, H, W]
                    if band_data.shape[1:] != (15, 15):
                        print(f"Warning: Unexpected shape at {file_path}: {band_data.shape}")
                        continue
                    # Ensure we only take the expected number of temporal samples
                    if band_data.shape[0] >= expected_samples:
                        data_array[band_idx] = band_data[:expected_samples]
                    else:
                        print(f"Warning: Insufficient temporal samples in {file_path}")
                        data_array[band_idx, :band_data.shape[0]] = band_data
            except Exception as e:
                print(f"Error loading band {band} for location {location_id}: {str(e)}")
                raise
    ...
    def _load_ground_truth(self, gt_paths):
        ...
        try:
            for path in gt_paths:
                with rasterio.open(path) as src:
                    ...
                    if self.temporal_mode == "monthly":
                        # Repeat yearly data for all months in that year
                        data = np.repeat(data[:, np.newaxis], 12, axis=1)  # [4, 12, 5, 5]
                        data = data.reshape(-1, 5, 5)  # [48, 5, 5]
                        # Take only the first 42 months (3.5 years)
                        data = data[:42]
            ...
            if not valid_data:
                raise ValueError(f"No valid ground truth data found")
    ...
    def __getitem__(self, idx):  
        location_id = self.unique_ids[idx]
        ...
        try:
            ...
            try:
                sentinel_data = self._load_sentinel(location_id)
            except Exception as e:
            ...
            print(f"Error loading data for {location_id}: {str(e)}")
            raise
