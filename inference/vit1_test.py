...
def calculate_bin_change_metrics(predictions, ground_truth, reference_months, bins=[0.0, 0.25, 0.5, 0.75, 1.0]):
    """
    Calculate change metrics for each bin value
    ...
    all_pred_changes = []
    all_truth_changes = []
    bin_masks = {bin_val: [] for bin_val in bins}
    ...
    for i in range(B):
        ...
        # Create bin masks for the original values (before changes)
        for bin_val in bins:
            # Get mask for original values matching the bin
            bin_mask = (torch.abs(loc_truth[..., :-1, :, :] - bin_val) < 1e-6)
            bin_masks[bin_val].append(bin_mask)
    ...
    if len(all_pred_changes) > 0:
        ...
        for bin_val in bins:
            if len(bin_masks[bin_val]) > 0:
                combined_mask = torch.cat(bin_masks[bin_val], dim=0)
                
                if torch.sum(combined_mask) > 0:
                    # Flatten and apply mask
                    bin_pred_changes = all_pred_changes.reshape(-1)[combined_mask.reshape(-1)]
                    bin_truth_changes = all_truth_changes.reshape(-1)[combined_mask.reshape(-1)]
                    ...
                    bin_change_metrics.append({
                        'bin': bin_val,
                        **metrics,
                        'count': torch.sum(combined_mask).item()
                    })
                else:
                    bin_change_metrics.append({
                        'bin': bin_val,
    ...
    if len(changing_predictions) > 0:
        ...
        bin_changing_combined_metrics = [
            {'bin': bin_val, 'r2': 0.0, 'rmse': 0.0, 'mae': 0.0, 'me': 0.0, 
             'r2_change': 0.0, 'rmse_change': 0.0, 'mae_change': 0.0, 'me_change': 0.0, 'count': 0}
            for bin_val in bins
        ]
...
def main():
    ...
    try:
        coords_path = "/mnt/guanabana/raid/hdd1/qinxu/Python/Data/Raw/validation-africa.csv"
    ...
    # Load checkpoint
    checkpoint_path = "/mnt/guanabana/raid/hdd1/qinxu/Python/LCF-ViT/training/monthly_15_best_model.pth"
